...
/// Instantly mints `amount` USDC on `toChain`, sent to `recipientAddress`,
/// out of the caller's unified Gateway balance. The SDK automatically
/// decides which underlying chain balances to burn from — no source chain
/// needs to be specified.
export async function spendFromGateway({ toChain, amount, recipientAddress }) {
  const adapter = await getAdapter();
  const kit = new UnifiedBalanceKit();
  return kit.spend({
    amount: String(amount),
